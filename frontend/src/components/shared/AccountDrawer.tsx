...
export function AccountDrawer({ open, onClose }: Readonly<AccountDrawerProps>) {
  ...
  return (
    <Drawer open={open} onClose={onClose} title="Account" subtitle="Profile and security">
          ...
          {openSections.has("profile") && (
            <div className="px-5 pb-5 border-t border-slate-100 pt-4">
              ...
              <div className="space-y-4">
                <div>
                  <label htmlFor="drawer-full-name" className="block text-sm font-medium text-slate-700 mb-1.5">Full Name</label>
                  <input
                    id="drawer-full-name"
                    type="text"
                    value={fullName}
                ...
                <div>
                  <label htmlFor="drawer-email" className="block text-sm font-medium text-slate-700 mb-1.5">Email</label>
                  <input
                    id="drawer-email"
                    type="email"
                    value={user?.email || ""}
        ...
        <section className="bg-white rounded-xl border border-slate-200 overflow-hidden">
          ...
          {openSections.has("password") && (
            <div className="px-5 pb-5 border-t border-slate-100 pt-4">
              ...
              <div className="space-y-4">
                <div>
                  <label htmlFor="drawer-current-pw" className="block text-sm font-medium text-slate-700 mb-1.5">Current Password</label>
                  <input id="drawer-current-pw" type="password" value={currentPassword} onChange={(e) => setCurrentPassword(e.target.value)} className="w-full px-3.5 py-2.5 text-sm border border-slate-200 rounded-lg bg-white focus:border-teal-500 focus:ring-1 focus:ring-teal-500" autoComplete="current-password" />
                </div>
                <div>
                  <label htmlFor="drawer-new-pw" className="block text-sm font-medium text-slate-700 mb-1.5">New Password</label>
                  <input id="drawer-new-pw" type="password" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} className="w-full px-3.5 py-2.5 text-sm border border-slate-200 rounded-lg bg-white focus:border-teal-500 focus:ring-1 focus:ring-teal-500" autoComplete="new-password" />
                </div>
                <div>
                  <label htmlFor="drawer-confirm-pw" className="block text-sm font-medium text-slate-700 mb-1.5">Confirm New Password</label>
                  <input id="drawer-confirm-pw" type="password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} className="w-full px-3.5 py-2.5 text-sm border border-slate-200 rounded-lg bg-white focus:border-teal-500 focus:ring-1 focus:ring-teal-500" autoComplete="new-password" />
                </div>
                <div className="pt-2">
                  <button
...
function Feedback({ state }: Readonly<{ state: NonNullable<FeedbackState> }>) {
  return (
    <div className={`mb-4 p-3 text-sm rounded-lg border flex items-center gap-2 ${
      ...
      {state.type === "success" ? <CheckCircle2 className="w-4 h-4 shrink-0" /> : <AlertCircle className="w-4 h-4 shrink-0" />}
      {state.message}
    </div>
  );
}
