...
export function StatusBadge({ label, tone = "slate", pulse = false }: StatusBadgeProps) {
  ...
  return (
    <span className={`inline-flex items-center gap-2 rounded-full border px-2.5 py-1 text-[11px] font-semibold ${t.bg} ${t.text} ${t.border}`}>
      <span className={`h-2 w-2 rounded-full ${t.dot} ${pulse ? "animate-pulse" : ""}`} />
      {label}
    </span>
  );
}
