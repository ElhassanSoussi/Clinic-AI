...
import { Search, Filter, Globe, Smartphone, Clock, CheckCircle2, Inbox as InboxIcon, Layers } from "lucide-react";
...
function InboxTriagePanel({
  ...
  afterQueueChange?: () => void;
}) {
  const choose = (m: FilterMode) => {
    setFilterMode(m);
    afterQueueChange?.();
  };
  ...
  return (
    <>
      <div className="p-5 border-b border-border">
        ...
        <p className="text-xs text-muted-foreground mt-1">Pick a queue, then work newest threads first.</p>
      </div>
      ...
      <div className="p-3 space-y-1">
        ...
        >
          All threads
          <span className="text-xs font-medium text-muted-foreground tabular-nums">{rawLength}</span>
        </button>
        <button
          type="button"
...
function InboxScanRhythm({ onJumpAttention }: { onJumpAttention: () => void }) {
  return (
    <>
      ...
      <ul className="text-sm text-muted-foreground space-y-3 leading-relaxed">
        <li>
          ...
          <button type="button" onClick={onJumpAttention} className="text-primary font-semibold hover:underline">
            Needs attention
          </button>{" "}
      ...
      <div className="h-full bg-background flex flex-col xl:flex-row min-h-0">
        ...
        <div className="flex-1 flex flex-col min-w-0 min-h-0 xl:flex-row">
              ...
              <div className="flex flex-col gap-3 mb-4">
                <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
                  ...
                  <div className="flex flex-wrap items-center gap-2 sm:justify-end sm:pt-1">
                    <button
                      type="button"
                      onClick={() => setTriageSheetOpen(true)}
                      className="xl:hidden inline-flex items-center gap-2 h-9 px-3 rounded-lg border border-border bg-white text-sm font-semibold text-foreground hover:bg-muted transition-colors"
                    >
                    ...
                    <div className="flex items-center gap-2 text-xs text-muted-foreground">
                      ...
                      <span>
                        {filterMode === "attention" ? "Attention queue only" : "Full inbox"}
                      </span>
                    </div>
                  </div>
                </div>
            ...
            <div className="flex-1 overflow-auto bg-background">
              <div className="divide-y divide-border">
                ...
                {!loading && conversations.length === 0 && (
                  <div className="p-6">
                    <div className="rounded-lg border border-dashed border-border bg-slate-50/60 px-4 py-6 text-center max-w-md mx-auto">
                      <p className="text-sm font-medium text-foreground">
                        {filterMode === "attention" && raw.length > 0
                          ? "No threads match this search in the attention queue"
                          : raw.length === 0
                      ...
                      <p className="text-sm text-muted-foreground mt-1">
                        {raw.length === 0
                          ? "When patients use your chat or SMS, threads will show up here. Share your public chat link from Settings."
                          : filterMode === "attention"
                            ? "Try clearing the search box or switch to All threads to see the full inbox."
                            : "Try clearing the search box or adjusting your filters."}
                      </p>
                    </div>
                  </div>
                )}
          ...
          <aside className="hidden xl:block w-72 flex-shrink-0 border-l border-border bg-white p-5 space-y-4">
            <InboxScanRhythm onJumpAttention={() => setFilterMode("attention")} />
          </aside>
        </div>
      </div>

      <Sheet open={triageSheetOpen} onOpenChange={setTriageSheetOpen}>
        <SheetContent side="bottom" className="max-h-[90vh] overflow-y-auto gap-0 rounded-t-xl p-0">
          <SheetHeader className="p-5 pb-3 text-left border-b border-border space-y-1">
            <SheetTitle className="text-lg">Triage & scan rhythm</SheetTitle>
            <SheetDescription>
              Same queues and guidance as the desktop sidebar — use this on smaller screens when the left column is compact.
            </SheetDescription>
          </SheetHeader>
          <div className="bg-white">
            <InboxTriagePanel
              ...
              rawLength={raw.length}
              attentionCount={attentionCount}
              afterQueueChange={() => setTriageSheetOpen(false)}
            />
          </div>
